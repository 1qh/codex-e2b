...
const codexAuth = readFileSync(`${homedir()}/.codex/auth.json`, 'utf8'),
  createSandbox = async () => {
    ...
    await sandbox.commands.run(`PATH=${home}/.bun/bin:$PATH bun install -g @openai/codex`, { timeoutMs: 120_000 })
  ...
  runCodex = async (opts: { home: string; onEvent: (event: unknown) => void; prompt: string; sandbox: Sandbox }) => {
    const handle = await opts.sandbox.commands.run(
      `cd ${opts.home} && PATH=${opts.home}/.bun/bin:$PATH codex exec --dangerously-bypass-approvals-and-sandbox --json "${opts.prompt.replaceAll('"', String.raw`\"`)}"`,
      {
        background: true,
        onStdout: data => {
